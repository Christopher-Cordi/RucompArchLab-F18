...
int ll_has_cycle(node *head) {
	/* your code here */
//set both pointers to head of the list
	 hare = head;
	tortoise = head;

//keep coming back to this step until we reach the end
while(hare == tortoise){
//advance hare twice through the list
	for(int i = 0; i < 2; i++){
	hare = hare->next;
	//if hare equals null we have reached the end
		if(hare == NULL){
		printf("This is acyclic");
		//break loop if this happens
		break;
		}
	}
	
	//we do not need to check tortoise since head is already 2 ahead
	tortoise = tortoise->next;
	//since this happened and it didnt break
